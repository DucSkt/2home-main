...
export default (state = INIT_STATE, action) => {
  switch (action.type) {
    case FETCH_API_EXPENSE_MONTH_TO_DATE_BY_ID:
      return {
        data: state.data,
        error: null,
        fetching: true
      };
    case FETCH_API_EXPENSE_MONTH_TO_DATE_BY_ID_SUCCESS:
      return createSuccessState(action,state);
    case FETCH_API_EXPENSE_MONTH_TO_DATE_BY_ID_FAILED:
      return {
        data: null,
        error: action.payload,
        fetching: false
      };
    case REFRESH_EXPENSE_MONTH_TO_DATE_DATA:
      return INIT_STATE;
    case RESET_STATE:{
        return INIT_STATE
    }
